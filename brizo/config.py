...
class Config(configparser.ConfigParser):
    ...
    def keeper_path(self):
        ...
        keeper_path_string = self.get(
            self._section_name, NAME_KEEPER_PATH, fallback=None
        )
        return (
            Path(keeper_path_string).expanduser().resolve()
            if keeper_path_string
            else ""
        )
