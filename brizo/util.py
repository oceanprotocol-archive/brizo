...
def get_env_property(env_variable, property_name):
    return getenv(env_variable, get_config().get("osmosis", property_name))
...
def get_latest_keeper_version():
    ...
    keeper_versions = sorted(
        [
            tuple([int(v) for v in c.version[1:].split(".")])
            for c in keeper.contract_name_to_instance.values()
            if c
        ]
    )
...
def validate_agreement_expiry(service_agreement, start_time):
    ...
    if current_time > expiry_time:
        agreement_exp = (
            datetime.fromtimestamp(expiry_time).replace(microsecond=0).isoformat()
        )
...
def get_asset_url_at_index(url_index, asset, account):
    logger.debug(
        f"get_asset_url_at_index(): url_index={url_index}, did={asset.did}, provider={account.address}"
    )
    try:
...
def validate_algorithm_dict(algorithm_dict, algorithm_did):
    ...
    if not algorithm_dict["url"] and not algorithm_dict["rawcode"]:
        return (
            f"`algorithmMeta` must define one of `url` or `rawcode`, but both seem missing.",
            400,
        )
    ...
    if not (
        ...
        return (
            f"algorithm `container` must specify values for all of entrypoint, image and tag.",
            400,
        )
