...
def test_consume(client):
    ...
    payload = dict(
        {"serviceAgreementId": agreement_id, "consumerAddress": cons_acc.address}
    )
    ...
    assert (
        event
    ), "Lock reward condition fulfilled event is not found, check the keeper node's logs"
    ...
    files_list = json.loads(
        do_secret_store_decrypt(
            did_to_id(ddo.did), ddo.encrypted_files, pub_acc, get_config()
        )
    )
...
def test_empty_payload(client):
    consume = client.get(
        BaseURLs.ASSETS_URL + "/consume", data=None, content_type="application/json"
    )
    assert consume.status_code == 400

    publish = client.post(
        BaseURLs.ASSETS_URL + "/publish", data=None, content_type="application/json"
    )
...
def test_auth_token():
    ...
    assert address and address.lower() == pub_address.lower(), (
