...
def get_template_actor_types(keeper, template_id):
    ...
    return [
        keeper.template_manager.get_template_actor_type_value(_id)
        for _id in actor_type_ids
    ]
...
def place_order(publisher_account, ddo, consumer_account, service_type):
    ...
    actors = [
        actor_map[_type] for _type in get_template_actor_types(keeper, template_id)
    ]

    assert keeper.template_manager.contract_concise.isTemplateIdApproved(
        template_id
    ), f"template {template_id} is not approved."
